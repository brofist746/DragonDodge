...
class Dragon:
    ...
    def __init__(self):
        self.dragon_img = pygame.image.load('drag.png')
...
class Ultron:
    ...
    def __init__(self):
        ...
        self.ultron_img1= pygame.image.load('ka.png')
        
        self.ultron_img1_rect = self.ultron_img1.get_rect()
        self.ultron_img1_rect.left = 10
        self.ultron_img1_rect.top =                    WINDOW_HEIGHT / 2 - 100      
        self.down = True
        self.up = False

    def update(self):
        canvas.blit(self.ultron_img1, self.ultron_img1_rect)
        if self.ultron_img1_rect.top <= cactus_img_rect.bottom:
            game_over()
            if SCORE > self.ultron_score:
                self.ultron_score = SCORE
        if self.ultron_img1_rect.bottom >= fire_img_rect.top:
            game_over()
            if SCORE > self.ultron_score:
                self.ultron_score = SCORE
        if self.up:
            self.ultron_img1_rect.top -= 10
        if self.down:
            self.ultron_img1_rect.bottom += 10
...
def game_over():
    ...
    music = pygame.mixer.Sound('ultron_dies.wav')
    music.play()
    topscore.top_score(SCORE)
    game_over_img = pygame.image.load('end1.png')
...
def game_loop():
    while True:
        ...
        global  HIGH_SCORE
        flames_list = []
        pygame.mixer.music.load('ultron_theme.wav')
        ...
        while True:
            ...
            ultron.update()
            for f in flames_list:
                if f.flames_img_rect.colliderect(ultron.ultron_img1_rect):
                    game_over()
                    if SCORE > ultron.ultron_score:
                        ultron.ultron_score = SCORE
            pygame.display.update()
            CLOCK.tick(FPS)
